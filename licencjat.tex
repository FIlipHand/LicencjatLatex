...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[
		...
		\node [label=$\mu$, fit=(mu-1) (mu-2), draw, fill=yellow, opacity=0.1] (mu) {};
		\node [label=$\sigma$, fit=(sigma-1) (sigma-2), draw, fill=red, opacity=0.1] (sigma) {};
		\node [label=$z$, fit=(sample-1) (sample-2), draw, fill=orange, opacity=0.1] (sample) {};
		...
		\node[above=0.1 of encoder-1-1] {wejście};
		\node[above=0.1 of decoder-3-1] {wyjście};
